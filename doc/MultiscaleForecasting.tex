...
\begin{document}
\title{Feature generation for multiscale time series forecasting multimodels\thanks{16-29-12912}}
...
$\Gs$, $\Grs$ & initial and resampled grid \\
$\Ts$, $\Trs$ & initial and resampled number of observations in $\bs$\\
$\fs = \frac{1}{\ts}$ & sampling rate \\
$\dtp$ & local prehistory \\
$\dtr$ & defines time period (requested) for prediction \\
$[\y^{(m)}_i|\x^{(m)}_i]$ & local ($t_i$) segment from the time series $\bs^{(m)}$ \\
...
$\bc^{(m)}_1,\dots,\bc^{(m)}_P$ & $k$-means centroids \\
...
Consider a large set of time series~$\fD=\{\bs^{(m)}| \; m = 1\dots, {M}\}$, where each real-valued time series~$\bs$
\[ \bs = [s_1, \dots, s_i, \dots, s_{T}], ~~ s_i = s(t_i),\quad 0 \leq t_i \leq t_{\max}\]
is a sequence of observations (\hl{measurements, records}) of some real-valued signal $s(t)$.
Each time series $\bs^{(m)}$ has its own number of observations $T^{(m)}$. Moreover, the sampling rate might be changing over time
\[ t_i \neq i\cdot\frac{t_T - t_1}{T-1}, \]
or the ratio between different sampling rates might be non-rational. \hl{How do we know??} In such cases as well as for time series with missing values we apply resampling procedure, which is described further.
...
\begin{figure}[!ht]
\centering\subfloat[]{\includegraphics[width=0.2\textwidth]{resample1.png}\label{fig_resample1}}
\centering\subfloat[]{\includegraphics[width=0.3\textwidth]{resample2.png}\label{fig_resample2}}
...
Suppose that the signal $s(t)$ is bandlimited with frequency $\fb$. According to the Nyquist–Shannon  sampling theorem, it is sufficient to sample the signal $s(t)$ with frequency \begin{equation}\label{eq:nyquist_condition}
\frac{1}{\trs} = \fs > 2\fb\end{equation}
to be able to fully reconstruct the signal from its discretely sampled measurements $s(t_i) = s(i\ts)$. Alternatively, for $\fs > 2\fb$, Whittaker-Shannon interpolation
\[s(t) = \sum_{i = -\infty}^{\infty} s_i\text{sinc}\left(\frac{t-i\ts}{\ts}\right)\]
of the time series $\bs$ yields perfect reconstruction of the signal $s(t)$. Sampling with $\fs < 2\fb$ causes distortions known as aliasing. In this case the time series have to be low-pass filtered to satisfy the Nyquist condition~\eqref{eq:nyquist_condition}.

\paragraph{FFT resampling.}
In signal processing a common way to change resolution of a signal is to use a combination of upsampling and decimation, implemented here via FFT transform and downsampling. Suppose that desired sampling rate $\fs$ is fixed, that is, we would like to approximate $\Ts = \fs t_{\max}$ uniformly sampled observations
\[{s}(i\ts), \; i = 1, \dots \Ts, \quad \text{where~} \ts = \frac{t_{\max}}{\Ts - 1},\]
of the signal $s(t)$. Let $\Gs = \{t_1, \dots, t_T\}$ and $\Grs = \{\ts, \dots, \ts\Ts\}$ denote the current and the desired grids.
The first step to resampling $s(\Gs) \rightarrow s(\Grs)$ is the piecewise approximation $\hat{\bs}$ of time series $\bs$ at equidistant time points $\Grs$ (see Fig.~\ref{fig_resample1} and Fig.~\ref{fig_resample2})
\begin{equation}\label{eq:piecewise_c}
%\hat{s}(\hat{t}_i) = s(t_i), \text{~where~} t_i = \max\{t|t\in \Gs ~\text{and~} t \leq t_i \}.
\hat{s}_i = s(t_i), \quad t_i = \max_{t\in \Gs ~\text{and~} t \leq i\trs} t.
\end{equation} To increase the smoothness of piecewise approximation $\hat{\bs}$ we apply low pass DFT filtering to $\hat{\bs}$, so that the reconstructed time series are bandlimited with
$\fb < \fs/2$ and then downsample the output to $\Grs$.

\hl{On selecting $\fs$ and $\fb$.}
Earlier we assumed that the desired sampling rate $\fs$ is fixed. In fact, the new grid $\Grs$ is defined by the number of sampled observations $\Ts$ rather than the sampling frequency.
Consider some special cases of unevenly sampled signals, where selecting $\Ts$ is straightforward.
\begin{enumerate}
...
\item The sampling rate is even, but some values are missing:
\[ |t_{i+1} - t_{i}| = n\tau, n \in \mathbb{N}.\]
Here $\ts = \tau$ and missing values are the only ones that one needs to approximate.
\item Time series $\bs$ comprises a finite number of intervals $\bs_k$, each sampled from $s(t)$ at fixed sampling rate:
...
\hl{The problem here is that we cannot estimate $\fb$ until the time series are evenly sampled, which means that $\fs$ has already been set and we only need to set $\fb$ to $\fs/2$.}
...
\begin{algorithm}[!ht]
\DontPrintSemicolon
\emph{\textbf{TimeSeriesResampling()}}\\
\Begin{
 \KwData{time series $\bs = s(\Gs)$, sampled at $\Gs = \{t_1, \dots, t_T\}$. }
 \textbf{Parameters:}{ Upsampling rate $\fs$.} \\
 \KwResult{resampled time series $\bs = [\tilde{s}(0), \dots,  \tilde{s}(t_{\Trs})]$.}
 Form the new grid $\Grs  = \{0, \dots, \Ts\cdot\ts\}$\;
 Use piecewise approximation~\eqref{eq:piecewise_c} to upsample time series: $\hat{\bs} =  \uparrow_{\Grs} \bs$ \;
 %$\quad \hat{\bs} = \text{~\textbf{Piecewise}}(\bs, \Gs, \Grs)$\;
  Apply low pass filtering to upsampled time series $\hat{\bs}$:

 $\quad \hat{\bs}^{\text{b}} = \text{\textbf{LowPassFFTFiltering}}(\hat{\bs}, \alpha)$ \;
 If necessary, downsample $\hat{\bs}_{\text{b}}$ to $\Grs$. }
 \bigskip
...
% \Begin{
% \KwData{Time series, $\bs$. Parameters: cut-off value $\fb$ for high frequencies.}
...
% Set $a_j = 0,\; b_j=0$, for  $2\pi w_j > \fb$ \;
% Reconstruct the time series, using inverse FFT.
%}
%or \\
\emph{\textbf{LowPassFFTFiltering()}} \\
 \Begin{
 \KwData{Time series, $\bs$. Parameters: tolerance to spectrum reduction level $\alpha$.}
 \KwResult{Filtered time series $\bs_{\text{b}}$.}
 Zero-pad ${\bs}$, so that $|\bs| = 2^{N}$, where $N = \lceil \log_2(|\bs|)\rceil$\;
 Find FFT coefficients $a_j,\; b_j$ for  $j = 1, \dots, N$ for $\bs$\;
 Set $\fb$ according to~\eqref{eq:bandlimit}\;
 Set $a_j = 0,\; b_j=0$, for  $2\pi w_j > \fb$ \;
 Reconstruct the time series, using inverse FFT.
}
 \smallskip
...
To reformulate the forecasting task into regression task, form an object set at a set of the vectors~$\{\bx^{*}\}$, where each vector~$\bx^{*} = [\by| \bx]$ collects all the time series over the time period~$\dtp$ (Fig.~\ref{fg:draw}), which stands for the local \emph{prehistory}. The vector $\bx^{*}$ includes samples from previous history of any time series $\bs^{(m)}$ as well as any derivatives $\vf$, which are called generated features.
...
\item and
scale-free error metrics, which express each error as a ratio to an average error from a
martingal forecast:
...
\paragraph{Strijov's version of testing procedure}
To test consequent series of forecasts introduce the rolling forecasting procedure. It computes forecasts over sequence of time segments. The testing procedure counts~$K$ segments. Each segment starts at time-point~$t_k$. It has a given length~$\dtr$. It varies from one time-tick to to the length of the forecast horizon, which estimated before the experiment. To construct the design matrices for each time-point~$t_k$ split time-scale into~$K$ segments~$\dtr$ up to the end. For each time-point~$t_k$ do the following:
\begin{enumerate}[1)]
...
\item construct the rest rows of the design matrix~$\bZ$ for the time after~$t_k$ and present it as
 ...
 \end{array}\right],
 \]
\item optimize model parameters~$(\bw,\beta)$ using~$\bX_{\text{train}, k}, \bY_{\text{train}, k}$, (run additional cross-validation procedure on there rows to select the optimal model structure, if necessary),
\item use~$\bx_{\text{val}, k},\by_{\text{val}, k}$ to compute the forecast~$\bf(\bx_\text{val}\bw,\beta)$ and its residues~$(\fx-\by)$,
\item compute~$\text{MAPE}$~\eqref{eq_MAPE}.
\end{enumerate}
...
p(\by|\x, \bth) = \sum_{k=1}^{K}\pi_k\;\mathcal{N}(\by|\w_k\T\bx,\beta) = \end{equation}
\[ \sum_{k=1}^K
...
\hat{\bth} = \argmax_{\bth}\ln p(\by|\bth), ~\ln p(\by|\bth) = \sum_{i=1}^{m}\ln\left( \sum_{k=1}^K  \pi_k\mathcal{N}(\by|\w\T_{k}\x_i,\beta) \right).
\end{equation}
...
variables, using~\eqref{eq:hidden_vars_Estep}\;
  ...
  $ \bv^{(r+1)}_k = \argmax_{\bv} Q^{(r),\bv}_k(\bv)$, where $Q^{(r),\bv}_k(\bv) = \sum_{i=1}^m \gamma_{ik}^{(r+1)}\ln\pi_k(\x_i, \bv)$ \\
  $ \w^{(r+1)}_k = \argmax_{\w_k} Q^{(r),\w}_k(\w_k)$, where $Q^{(r),\w}_k(\w_k) = \sum_{i=1}^m\gamma^{(r+1)}_{ik}
    \left(
        y_i-\w_k\T\x_i
    \right)^2, $ \\
  $ \beta^{(r+1)}_k = \argmax_{\beta} Q^{(r),\beta}_k(\beta)$,
  where $ Q^{(r),\beta}_k(\beta) =\left(n\ln\beta
        -\frac{1}{\beta}(y_i-\x_i\T\w^{(r+1)}_k)^2
    \right)
    \;$
 }
