...
The table~\ref{tb:primitives} lists the~time series transformation functions. There are non-parametric and parametric procedures to generate features. For the parametric functions~$g=g(\bb,s)$ the default values of the parameters~$\bb$ are assigned empirically.
...
\]
This procedure repeats two problems until vectors~$\hat\bw,\hat\bb$ converge. The initial values of vector~$\bb$ (are shown in table~\ref{tb:primitives}). Due to the various origins of the time series and their transformations the residual vector should be normalized:
...
\begin{table}[!ht]
\caption{List of primitive functions.}
\label{tb:primitives}
...
Hyperbolic tangent	&	$	\tanh(x)	$	&	1	&	1	&	0	\\ \hline
Hyperbolic tangent sigmoid	&	$	\frac{2}{1 - \exp(-wx)} - 1	$	&	1	&	1	&	1	\\ \hline
...
\begin{table}[!ht]
\caption{Growth rate of monotone functions.}
\label{tb:monotone}
...
\begin{table}[!hb]
\caption{Variable number of parameters.}
\label{tb:variable}
\begin{tabular}{|p{90pt}|c|p{50pt}|p{50pt}|p{50pt}|}
\hline
Quasi-linear	&	$	\sum_{j=1}^n a_jg_j(x)	$	&	1	&	1	&	$n$	\\ \hline
Sum of Gaussians	&	$	\sum_{j=1}^n a_j\exp(-\frac{(x-b_j)^2}{c_j})	$	&	1	&	1	&	 $3n$	 \\ \hline
