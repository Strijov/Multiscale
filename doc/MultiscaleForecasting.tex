...
\Begin{
 ...
 \KwResult{resampled time series $\bs(\Grs) = \{\tilde{s}(\tilde{t}_i)| \tilde{t}_i\in \Grs \}$.}
 ...
 $\quad \hat{\bs}_{\text{lf}} = \text{\textbf{LowPassFFTFiltering}}(\hat{\bs}, \frac{\fs}{2} - \varepsilon)$ \;
...
\hline
\bx_0^{(1)} & \dots & \bx_0^{(M)} & \by_0^{(1)} &  \dots & {\by}_0^{(M)}   \\
\end{array}
\right]
=\left[
\begin{array}{c|c}
