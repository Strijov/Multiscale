...
Suppose that a signal $s(t)$ is bandlimited with frequency $f_b$. According to the Nyquist–Shannon  sampling theorem, it is sufficient to sample the signal with frequency \begin{equation}\label{eq:nyquist_condition}
...
\textbf{\emph{ComputeForecastingErrors()}}
 ...
 define, $\bX^{*}_n = [\bx^{*}_{n}, \dots, \bx^{*}_{m + n - 1}]\T$ \;
  ...
  $n = n + 1$ \;
  (or any adequate error function from~\eqref{par:frc_errors}) \\
  }
