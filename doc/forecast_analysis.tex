...
\textbf{\emph{ComputeForecastingErrors()}}
 ...
 define  $m = \lfloor M/N\rfloor$ $\bX^{*}_n = [\bx^{*}_{(n-1)m +1}, \dots, \bx^{*}_{nm}]\T$ \;
  ...
  train forecasting model $\fx(\x, \hat{\w})$, using $\bX_{\text{train}}$ and $\bX_{\text{test}}$\;
  obtain vector of residuals $\veps = [\varepsilon_{T}, \dots, \varepsilon_{T-K\Delta t - \dtr}]$ with respect to $\bX_{\text{val}}$:
...
\KwData{Object-feature matrix $\bX \in \mathbb{R}^{m\times(\dtr+\dtp)}$. Train to test ratio $\alpha$.}
 ...
 \hline
 \bX^{*}_{m_{\text{train}}} \in \mathbb{R}^{m_{\text{test}} \times (\dtr + \dtp)} 
