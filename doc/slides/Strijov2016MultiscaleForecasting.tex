...
\Wider[3em]{
Expectation-Maximization algorithm maximizes $\mathsf{E}_{Z} [p(\y, Z|X, \bth)]$, updating parameters estimates $\bth^{(r)}$ in two iterative steps.

\vspace{3em}

{\bf E-step}: obtain $\mathsf{E}(Z) = \Gamma=[\gamma_{ik}]$. Using Bayesian rule, obtain:
\[
...
{\bf M-step:} update parameters $\bth$, maximizing $Q^{(r)}(\bth)$. Maximize function~$Q^{(r)}(\bth)$ with respect to~$\bth$ with~$\Gamma^{(r+1)}$ fixed:
